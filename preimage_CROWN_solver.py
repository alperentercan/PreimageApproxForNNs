...
class LiRPAConvNet:
    def __init__(
        ...
        self.device = device or preimage_arguments.Config["general"]["device"]
        self.net = BoundedModule(
            net, torch.zeros(in_size, device=self.device),
            bound_opts={
                'relu': 'adaptive',
                'deterministic': preimage_arguments.Config["general"]["deterministic"],
                'conv_mode': preimage_arguments.Config["general"]["conv_mode"],
                'sparse_features_alpha': preimage_arguments.Config["general"]["sparse_alpha"],
                'sparse_spec_alpha': preimage_arguments.Config["general"]["sparse_alpha"],
                'crown_batch_size': preimage_arguments.Config["solver"]["crown"]["batch_size"],
                'max_crown_size': preimage_arguments.Config["solver"]["crown"]["max_crown_size"],
                'forward_refinement': preimage_arguments.Config["solver"]["forward"]["refine"],
                'dynamic_forward': preimage_arguments.Config["solver"]["forward"]["dynamic"],
                'forward_max_dim': preimage_arguments.Config["solver"]["forward"]["max_dim"],
                'use_full_conv_alpha': preimage_arguments.Config["solver"]["alpha-crown"]["full_conv_alpha"],
            },
            device=self.device
        )
        self.net.eval()
        ...
        if transfer_items == "all":
            for cpu_layer, layer in zip(cpu_net.perturbed_optimizable_activations, net.perturbed_optimizable_activations):
                if hasattr(layer, 'sparse_beta') and layer.sparse_beta is not None:
                    if preimage_arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
    ...
    def get_candidate(self, model, lb, ub):
        ...
        # by lb and ub test_arguments (which is always passed in)
    ...
    def get_slope(self, model, only_final=False):
        ...
        kept_layer_names.extend(filter(lambda x: len(x.strip()) > 0, preimage_arguments.Config["bab"]["optimized_intermediate_layers"].split(",")))
    ...
    def update_bounds_parallel(
        ...
        if beta is None:
            beta = preimage_arguments.Config["solver"]["beta-crown"]["beta"] # might need to set beta False in FSB node selection
        optimizer = preimage_arguments.Config["solver"]["beta-crown"]["optimizer"]
        iteration = preimage_arguments.Config["solver"]["beta-crown"]["iteration"]
        lr_alpha = preimage_arguments.Config["solver"]["beta-crown"]['lr_alpha']
        lr_beta = preimage_arguments.Config["solver"]["beta-crown"]["lr_beta"]
        lr_decay = preimage_arguments.Config["solver"]["beta-crown"]["lr_decay"]
        get_upper_bound = preimage_arguments.Config["bab"]["get_upper_bound"]
        enable_opt_interm_bounds = preimage_arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']
        pruning_in_iteration = preimage_arguments.Config["bab"]["pruning_in_iteration"]
        pruning_in_iteration_threshold = preimage_arguments.Config["bab"]["pruning_in_iteration_ratio"]
        cut_iteration = preimage_arguments.Config["bab"]["cut"]["bab_iteration"]
        lr_cut_beta = preimage_arguments.Config["bab"]["cut"]["lr_beta"]
        cut_lr = preimage_arguments.Config["bab"]["cut"]["lr"]
        func_time = time.time()
        ...
        if beta:
            ...
            self.net.cut_used = preimage_arguments.Config["bab"]["cut"]["enabled"] and preimage_arguments.Config["bab"]["cut"]["bab_cut"]
        ...
        if fix_intermediate_layer_bounds:
            ...
            kept_layer_names = list(filter(lambda x: len(x.strip()) > 0, preimage_arguments.Config["bab"]["optimized_intermediate_layers"].split(",")))
    ...
    def update_bounds_cut_naive(
        ...
        beta = preimage_arguments.Config["solver"]["beta-crown"]["beta"]
        optimizer = preimage_arguments.Config["solver"]["beta-crown"]["optimizer"]
        cut_iteration = preimage_arguments.Config["bab"]["cut"]["iteration"]
        cut_lr_decay = preimage_arguments.Config["bab"]["cut"]["lr_decay"]
        cut_lr_beta = preimage_arguments.Config["bab"]["cut"]["lr_beta"]
        lr_alpha = preimage_arguments.Config["solver"]["beta-crown"]["lr_alpha"]
        lr_intermediate_beta = preimage_arguments.Config["solver"]["intermediate_refinement"]["lr"]
        opt_coeffs = preimage_arguments.Config["solver"]["intermediate_refinement"]["opt_coeffs"]
        opt_bias = preimage_arguments.Config["solver"]["intermediate_refinement"]["opt_bias"]
        early_stop_patience = preimage_arguments.Config["solver"]["early_stop_patience"]
        start_save_best = preimage_arguments.Config["solver"]["start_save_best"]
        cut_early_stop_patience = preimage_arguments.Config["bab"]["cut"]["early_stop_patience"]
        use_patches_cut = preimage_arguments.Config["bab"]["cut"]["patches_cut"]
        cut_reference_bounds = preimage_arguments.Config["bab"]["cut"]["cut_reference_bounds"]
        fix_intermediate_bounds = preimage_arguments.Config["bab"]["cut"]["fix_intermediate_bounds"]
        ...
        if not preimage_arguments.Config["bab"]["cut"]["bab_cut"]:
    ...
    def set_cuts(self, A, x, lower_bounds, upper_bounds, use_float64_in_last_iteration=False):
        ...
        cut_method = preimage_arguments.Config["bab"]["cut"]["method"]
        number_cuts = preimage_arguments.Config["bab"]["cut"]["number_cuts"]

        if preimage_arguments.Config["bab"]["cut"]["cplex_cuts"] and self.mip_building_proc is None:
            self.cutter = Cutter(self, A, x, number_cuts=number_cuts, device=self.net.device)

        cuts = None

        if cuts is None and not preimage_arguments.Config["bab"]["cut"]["cplex_cuts"]:
    ...
    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None, stop_criterion_func=stop_criterion_sum(0), bounding_method=None):
        ...
        lr_init_alpha = preimage_arguments.Config["solver"]["alpha-crown"]["lr_alpha"]
        init_iteration = preimage_arguments.Config["solver"]["alpha-crown"]["iteration"]
        share_slopes = preimage_arguments.Config["solver"]["alpha-crown"]["share_slopes"]
        lr_decay = preimage_arguments.Config["solver"]["alpha-crown"]["lr_decay"]
        optimizer = preimage_arguments.Config["solver"]["beta-crown"]["optimizer"]
        loss_reduction_func = preimage_arguments.Config["general"]["loss_reduction_func"]
        use_float64_in_last_iteration = preimage_arguments.Config["solver"]["use_float64_in_last_iteration"]
        early_stop_patience = preimage_arguments.Config["solver"]["early_stop_patience"]
        start_save_best = preimage_arguments.Config["solver"]["start_save_best"]
        bounding_method = bounding_method or preimage_arguments.Config["solver"]["bound_prop_method"]

        self.x = x
        self.input_domain = input_domain
        loss_reduction_func = reduction_str2func(loss_reduction_func)
        # self._set_A_options(get_upper_bound=False)
        self._set_A_options(get_upper_bound=True)


        if preimage_arguments.Config["bab"]["cut"]["enabled"]:
            self.return_A = True
        ...
        self.net.set_bound_opts({'verbosity': 3})
        self.net.set_bound_opts({'optimize_bound_args': {
        ...
        if bounding_method == "alpha-crown":
            ...
            if preimage_arguments.Config["attack"]["pgd_order"] == "middle" and vnnlib is not None:
                crown_filtered_constraints = np.zeros(len(lb[-1]))
                for i in range(len(lb[-1])):
                    if isinstance(preimage_arguments.Config["bab"]["decision_thresh"], torch.Tensor):
                        if preimage_arguments.Config["bab"]["decision_thresh"].shape[0] > 1:
                            if lb[-1][i].item() > preimage_arguments.Config["bab"]["decision_thresh"][i].item():
                                crown_filtered_constraints[i] = True
                        else:
                            if lb[-1][i].item() > preimage_arguments.Config["bab"]["decision_thresh"][0].item():
                                crown_filtered_constraints[i] = True
                    else:
                        if lb[-1][i].item() > preimage_arguments.Config["bab"]["decision_thresh"]:
                            crown_filtered_constraints[i] = True
            ...
            c_to_use = self.c
            if preimage_arguments.Config["solver"]["prune_after_crown"]:
                prune_after_crown_overhead = 0.
                stime = time.time()
                onedim_decision_thresh = preimage_arguments.Config["bab"]["decision_thresh"]
            ...
            with torch.no_grad():
                ret = self.net.compute_bounds(x=(x,), C=self.c, method=bounding_method, 
                return_A=self.return_A, needed_A_dict=self.needed_A_dict, bound_upper=False, cutter=self.cutter)
                if self.return_A:
                    lb, ub, A = ret
                    ...
                    lb, ub, pre_relu_indices = self.get_candidate(self.net, lb, lb + np.inf)
            # NOTE here return A instead of lA
        ...
        if preimage_arguments.Config["bab"]["cut"]["enabled"] and preimage_arguments.Config["bab"]["cut"]["cplex_cuts"] and self.mip_building_proc is None:
            ...
            self.cutter = Cutter(self, A, x, number_cuts=preimage_arguments.Config["bab"]["cut"]["number_cuts"], device=self.net.device)
    ...
    def copy_alpha(
        ...
        opt_intermediate_beta  = False
        opt_interm_bounds = preimage_arguments.Config["solver"]["beta-crown"]["enable_opt_interm_bounds"]
        for m in self.net.relus:
    ...
    def build_the_model_with_refined_bounds(self, input_domain, x, refined_lower_bounds, refined_upper_bounds,
        ...
        get_upper_bound = preimage_arguments.Config["bab"]["get_upper_bound"]
        use_float64_in_last_iteration = preimage_arguments.Config["solver"]["use_float64_in_last_iteration"]
        target_batch_size = preimage_arguments.Config["solver"]["multi_class"]["label_batch_size"]
        start_save_best = preimage_arguments.Config["solver"]["start_save_best"]
        opt_intermediate_beta  = False
        ...
        for now_batch in range(tot_batches):
            ...
            self.net.set_bound_opts({'optimize_bound_args': {
                ...
                'fix_intermediate_layer_bounds': False, 'lr_alpha': lr_init_alpha, 'init_alpha': False,
                'loss_reduction_func': loss_reduction_func,
                'stop_criterion_func': stop_criterion_func,
                'lr_decay': lr_decay, 'use_float64_in_last_iteration': use_float64_in_last_iteration,
                'start_save_best': start_save_best}})
            ...
            if all_slope_initialized and preimage_arguments.Config["solver"]["multi_class"]["skip_with_refined_bound"] is True:
            ...
            if preimage_arguments.Config["bab"]["attack"]["enabled"]:
    ...
    def get_lower_bound_naive(
        ...
        # NOTE: add lr_init_alpha to make it consistnet. change fix_inter_bounds to False, set use shared alpha to false
        # lr_init_alpha = test_arguments.Config["solver"]["alpha-crown"]["lr_alpha"]
        stop_criterion_func=stop_criterion_sum(0)
        self.net.set_bound_opts({'optimize_bound_args': {
            'iteration': preimage_arguments.Config["solver"]["alpha-crown"]["iteration"],
            'enable_beta_crown': False, 'enable_alpha_crown': True, 'single_node_split': True,
            'use_shared_alpha': preimage_arguments.Config["solver"]["alpha-crown"]["share_slopes"], 
            'optimizer': preimage_arguments.Config["solver"]["beta-crown"]["optimizer"],
            'early_stop': False,
            'keep_best': True,
            'fix_intermediate_layer_bounds': False,
            'lr_alpha': preimage_arguments.Config["solver"]["alpha-crown"]["lr_alpha"], 
            'init_alpha': False,
            'lr_decay': preimage_arguments.Config["solver"]["alpha-crown"]["lr_decay"],
            # 'lr_alpha': test_arguments.Config["solver"]["beta-crown"]['lr_alpha'],
            'stop_criterion_func': stop_criterion_func,
        }})
        ...
        return lb, [None] * (batch * 2), ret_s, lA, A_dict
